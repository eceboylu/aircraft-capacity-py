...
# ========================================================================
# ADIM (International Departure Split Graphs) - TAM 5 ana grafik
# (Overall/Domestic Security/Intl Departure—Passport/Intl Departure—
# Security/Intl Arrival), altıncı ("domestic arrival") yok.
# ========================================================================

def test_render_content_wires_exactly_the_five_required_graph_keys():
    script = _script()
    ...
    for key, data_field in (
        ...
        ("international_departure_passport", "data.international_departure.passport"),
        ("international_departure_security", "data.international_departure.security"),
        ("international_arrival", "data.international_arrival"),
    ):
        assert 'graphSectionHtml("' + key + '"' in render_fn
        assert data_field in render_fn

    # Altıncı ("domestic arrival"/"domestic_arrival") bir grafik YOK.
    assert "domestic_arrival" not in render_fn
    assert render_fn.count("graphSectionHtml(") == 5
    assert render_fn.count("drawChart(") == 5


def test_international_departure_passport_and_security_are_independent_titled_panels():
    script = _script()
    render_fn = script.split("function renderContent() {", 1)[1].split("\n  function ", 1)[0]

    assert 'graphSectionHtml("international_departure_passport", "INTERNATIONAL DEPARTURE — PASSPORT", data.international_departure.passport)' in render_fn
    assert 'graphSectionHtml("international_departure_security", "INTERNATIONAL DEPARTURE — SECURITY", data.international_departure.security)' in render_fn
    assert 'graphSectionHtml("overall", "GENEL HAVALİMANI YOĞUNLUĞU", data.overall)' in render_fn
    assert 'graphSectionHtml("domestic_security", "DOMESTIC SECURITY", data.domestic_security)' in render_fn
    assert 'graphSectionHtml("international_arrival", "INTERNATIONAL ARRIVAL", data.international_arrival)' in render_fn


def test_graph_section_html_no_longer_takes_a_kind_parameter():
    """`kind`/"breakdown" ayrımı kaldırıldı - her panel artık kendi tam process_series'ini alan tek-şekilli bir çağrı."""
    script = _script()
    assert "function graphSectionHtml(key, title, section) {" in script
    assert "function graphSectionHtml(key, title, section, kind)" not in script
...
def test_state_detail_has_five_independent_keys_initialized_to_null():
    script = _script()
    ...
    # Eski (artık geçersiz) anahtarlar YOK - tek birleşik
    # `international_departure` anahtarı split'le kalktı.
    assert "international_security:" not in detail_block
    assert "international_passport:" not in detail_block
    assert "international_departure: null" not in detail_block
...
def test_each_graph_click_handler_is_scoped_to_its_own_key_closure():
    ...
    assert 'drawChart(data.international_departure.passport.windows, "chart-international_departure_passport", "international_departure_passport")' in render_fn
    assert 'drawChart(data.international_departure.security.windows, "chart-international_departure_security", "international_departure_security")' in render_fn
    assert 'drawChart(data.international_arrival.windows, "chart-international_arrival", "international_arrival")' in render_fn


# ========================================================================
# ADIM (International Departure Split Graphs) - Bölüm 9-14: passport ve
# security artık İKİ AYRI panel/grafik - `waitDetailHtml` tek bir
# pencerenin wait'ini gösterir, iki süreç ARTIK BURADA birleştirilmez
# (birleştirme tamamen kaldırıldı, bkz. api.py
# `_international_departure_split`). Sahte toplama/ortalama YOK.
# ========================================================================

def test_wait_detail_html_no_longer_merges_passport_and_security():
    script = _script()
    fn = script.split("function waitDetailHtml(displayWindow) {", 1)[1].split("\n  }", 1)[0]

    assert "Tahmini bekleme süresi" in fn
    assert "waitTimeText(displayWindow)" in fn
    # Eski birleşik-breakdown dalı (kind==="breakdown") tamamen KALKTI.
    for forbidden in (
        'kind === "breakdown"', "Passport bekleme", "Security bekleme",
        "displayWindow.passport", "displayWindow.international_security",
        "/ 2", "average", "ortalama",
    ):
...
def test_frontend_never_renders_passenger_or_flight_counts():
    script = _script()
    graph_fn = script.split("function graphSectionHtml(key, title, section) {", 1)[1].split("\n  }", 1)[0]
    wait_fn = script.split("function waitDetailHtml(displayWindow) {", 1)[1].split("\n  }", 1)[0]
